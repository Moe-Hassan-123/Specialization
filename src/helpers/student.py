...
class Student:
    ...
    def __init__(
        ...
        """
        if (Student.isexist(national_id, password)) is not False:
    ...
    def password(self, password: str):
        if type(password) is not str:
            return TypeError
        pattern = r"(?=.*[0-9])(?=.*[(a-z)(\u0621-\u064A)(\u0660-\u0669)]).{8,32}"
    ...
    def isexist(national_id: str, password: str) -> int | bool:
    ...
    def fetch(id: int) -> dict:
        ...
        return fetch_as_dict(
            Student.c,
            "SELECT name, grade, specialization FROM students WHERE id = ?",
            id,
        )[0]

    def fetch_name(id: int) -> str:
        """returns the name of the student

        Args:
            id (int): the id of the student in the db
